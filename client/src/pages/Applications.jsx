            ...
            {applicants.filter(item => item.jobId && item.userId).map((application, index) => (
              <tr key={application._id} className="border-b hover:bg-gray-50">
                ...
                <td className="py-2 px-4 text-center flex items-center gap-2">
                  <img src={application.userId.image} alt="" className="w-10 h-10 rounded-full max-[875px]:hidden" />
                  <p>
                    {application.userId.name}
                  </p>
                </td>

                <td className="py-2 px-4 max-sm:hidden">{application.jobId.title}</td>
                <td className="py-2 px-4 max-sm:hidden">{application.jobId.location}</td>

                <td className="py-3">
                  <a href={application.userId.resume} target='_blank' className="text-blue-500 font-medium flex gap-2 bg-blue-50 px-4 py-2 rounded-lg">
                    Resume
